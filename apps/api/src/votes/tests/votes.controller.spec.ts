...
describe('VotesController', () => {
  ...
  describe('getVoteCounts', () => {
    it('should return vote counts for entity', async () => {
      ...
      expect(await controller.getVoteCounts(EntityType.TUTORIAL, 1)).toBe(result);
      expect(service.getVoteCounts).toHaveBeenCalledWith(EntityType.TUTORIAL, 1);
    });
  });

  describe('changeVote', () => {
    it('should change user vote', async () => {
      const result = { id: 1, userId: 1, entityType: EntityType.TUTORIAL, entityId: 1, voteType: VoteType.UP };
      mockVotesService.changeVote.mockResolvedValue(result);

      expect(await controller.changeVote(1, EntityType.TUTORIAL, 1, VoteType.UP)).toBe(result);
      expect(service.changeVote).toHaveBeenCalledWith(1, EntityType.TUTORIAL, 1, VoteType.UP);
    });
  });
  ...
  describe('removeUserVote', () => {
    it('should remove user vote', async () => {
      ...
      expect(service.removeUserVote).toHaveBeenCalledWith(1, EntityType.TUTORIAL, 1);
    });
  });
});